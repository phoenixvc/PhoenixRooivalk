...
**Issue**: `resourceManager` is accessed on line 105 before it's declared on line 118. This violates React Hooks rules and can cause runtime errors.
```typescript
...
```
**Fix**: Move the `resourceManager` declaration above the `waveManager` initialization.

### 2. **Memory Leak: CSS Styles Not Cleaned Up on Error**
**Severity**: Medium  
**File**: `src/components/WasmThreatSimulator.tsx:308-312`  
**Issue**: When WASM style loading fails, the cleanup function in the useEffect doesn't run, potentially leaving orphaned style elements in the DOM.
```typescript
} catch (err) {
...
**Issue**: The ID restoration uses `setTimeout` with a fixed 100ms delay, which may not be sufficient for slow devices or may be unnecessarily long for fast devices.
```typescript
setTimeout(() => {
...
```
**Fix**: Use a callback-based approach or wait for a specific WASM initialization event rather than arbitrary timeout.

### 4. **Singleton Pattern Broken on Component Remount**
**Severity**: Medium  
**File**: `src/components/WasmThreatSimulator.tsx:14, 170`  
**Issue**: The module-level `wasmInstanceInitialized` flag is reset in the cleanup, but if the component remounts immediately (e.g., during fast refresh), the WASM module itself may still be loaded in memory.
```typescript
...
**Issue**: If the WasmThreatSimulator component throws an error during initialization, it could crash the entire page with no graceful fallback.
```tsx
<WasmThreatSimulator />
```
...
**Improvement**: Memoize the plugin creation since it only depends on `uniqueMountId`
```typescript
