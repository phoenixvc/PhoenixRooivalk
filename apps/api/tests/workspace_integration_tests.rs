...
use phoenix_keeper::{ensure_schema, JobError, JobProvider, JobProviderExt, SqliteJobProvider};
use serde_json::json;
...
async fn test_keeper_job_processing() {
    ...
    let status: String =
        sqlx::query_scalar("SELECT status FROM outbox_jobs WHERE id = 'keeper-test-job'")
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn test_connection_management() {
    ...
    let health = phoenix_api::connection::HealthChecker::check_health(manager.pool())
        .await
        .unwrap();
    ...
    assert!(stats.size >= 1);
    // stats.active and stats.idle are unsigned, so they're always >= 0
    let _ = stats.active;
    let _ = stats.idle;
}
...
async fn test_api_to_keeper_cross_app_flow() {
    ...
    let tx_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM outbox_tx_refs WHERE job_id = ?1")
        .bind(&job_id)
        .fetch_one(&pool)
        .await
        .unwrap();
...
async fn test_api_to_keeper_failure_and_retry() {
    ...
    let failed_job = repo
        .get_evidence_by_id("retry-test-001")
        .await
        .unwrap()
        .unwrap();
