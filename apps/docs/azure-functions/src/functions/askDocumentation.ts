...
async function findRelevantChunks(
  ...
  const chunks = await queryDocuments<DocChunk>('doc_embeddings', query, params);
...
async function generateAnswer(
  ...
  const context = relevantChunks
    .map((chunk, i) => `[Source ${i + 1}: ${chunk.title} - ${chunk.section}]\n${chunk.content}`)
    .join('\n\n');
...
${context}`;
  ...
  if (history) {
    for (const msg of history.slice(-4)) { // Last 4 messages for context
  ...
  const avgSimilarity = relevantChunks.reduce((sum, c: any) => sum + c.similarity, 0) / relevantChunks.length;
...
async function handler(
  ...
  try {
    ...
    const relevantChunks = await findRelevantChunks(questionEmbedding, category);

    if (relevantChunks.length === 0) {
      return {
        status: 200,
        jsonBody: {
          answer: "I couldn't find relevant documentation to answer your question. Please try rephrasing or ask about a different topic.",
          sources: [],
          confidence: 'low',
        },
      };
