...
import { MARKET, PERFORMANCE, PRICING, FUNDING, BLOCKCHAIN, TEAM, PHASES } from "@site/src/data/values";
...
**Market Opportunity**: {MARKET.CURRENT} (2025) growing to {MARKET.PROJECTED} (2030) at {MARKET.CAGR}
CAGR
**Competitive Advantage**: 25-40x faster response times ({PERFORMANCE.RESPONSE_TIME} vs 3-10s
industry average)
**Technology Differentiation**: {PERFORMANCE.AUTONOMY_LEVEL} edge autonomy, blockchain evidence,
...
- **Base System Units**: {PRICING.SYSTEM_RANGE} per unit depending on configuration
...
- **Swarm Capability**: Handles {PERFORMANCE.CONCURRENT_TARGETS} concurrent threats
...
- **{BLOCKCHAIN.PRIMARY_CHAIN} Foundation**: Blockchain infrastructure, developer support
...
- **Export Markets**: Middle East and Asia-Pacific ({MARKET.ASIA_PACIFIC_CAGR} CAGR) with less
...
- Performance advantages: detection range over 5km, response time under 200
  milliseconds
- Success rate over 95%, multi-target capacity handling {PERFORMANCE.CONCURRENT_TARGETS} simultaneous threats
- Autonomous operation with minimal operator intervention through pre-authorized
  engagement protocols

### Development Roadmap

**{PHASES.SERIES_A.shortName}**: DoD validation through SBIR/STTR and OTA contracts targeting {PHASES.SERIES_A.funding}
in development funding

**{PHASES.SERIES_B.shortName}**: Production scale with IDIQ contracts and FMS
targeting {PHASES.SERIES_B.funding} annual revenue through partnerships with prime integrators

**{PHASES.SCALE.shortName}**: Commercial expansion post-regulatory changes targeting $100M+
