...
This appendix provides the detailed technical analysis supporting [ADR 0013: Identity & AuthN/AuthZ Strategy](./adr-0013-identity-auth.md).
...
async function verifyAccess(
  ...
  if (userLevel < requiredLevel) {
    throw new functions.https.HttpsError('permission-denied', `${requiredRole} role required`);
  }
  ...
  if (requiredFeature) {
    ...
    if (!features.includes(requiredFeature)) {
      throw new functions.https.HttpsError('permission-denied', `${requiredFeature} feature required`);
    }
  }
