...
if (!docsPath || !fs.existsSync(docsPath)) {
  console.error(
    "Error: Could not find docs folder. Use --docs-path to specify.",
  );
  process.exit(1);
}
...
if (!functionsUrl && !dryRun) {
  console.error(
    "Error: --functions-url is required (or set AZURE_FUNCTIONS_BASE_URL)",
  );
  process.exit(1);
}
...
async function indexDocs() {
  ...
  if (dryRun) {
    ...
    for (const doc of documents) {
      console.log(
        `  - ${doc.title} (${doc.category}) - ${doc.content.length} chars`,
      );
    }
  ...
  for (let i = 0; i < documents.length; i += batchSize) {
    ...
    console.log(
      `Indexing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(documents.length / batchSize)}...`,
    );
