...
1. **Cross-Origin-Opener-Policy (COOP) blocking OAuth popups**
   ...
   - OAuth popup authentication (Google/GitHub) could not communicate with parent window

2. **Empty JSON responses causing parse errors**
   - Error: `Uncaught (in promise) SyntaxError: Failed to execute 'json' on 'Response': Unexpected end of JSON input`
   - API calls were failing silently or returning empty bodies

## Root Causes

### 1. Restrictive COOP Header
- `staticwebapp.config.json` had `Cross-Origin-Opener-Policy: same-origin-allow-popups`
- This policy was too restrictive and blocked OAuth popup communication
- OAuth providers (Google, GitHub) open popups that need to communicate back to parent window

### 2. Missing JSON Response Handling
- `database.ts` called `response.json()` directly without checking if response had content
...
The bicep template (`infra/azure/modules/functions.bicep`) already has proper CORS configuration:
```bicep
cors: {
  allowedOrigins: [
    'http://localhost:3000'
...
- `deploy-docs-azure.yml` - Deploys docs site with updated `staticwebapp.config.json`
...
2. `apps/docs/azure-functions/src/functions/cosmos-proxy.ts` - Added CORS support
3. `apps/docs/azure-functions/src/lib/utils/responses.ts` - Centralized CORS helpers
4. `apps/docs/azure-functions/src/lib/utils/index.ts` - Export new utilities
5. `apps/docs/src/services/cloud/azure/database.ts` - Improved JSON parsing + credentials
6. `apps/docs/src/services/cloud/azure/functions.ts` - Added credentials to fetch
