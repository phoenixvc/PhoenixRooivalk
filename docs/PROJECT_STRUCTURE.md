...
All tooling configuration files are stored in the `config/` directory and
symlinked to the root for tool compatibility:
...
- **Markdownlint** uses `.markdownlint.json` (symlinked from
  `config/markdownlint.json`)

### Why Symlinks?

Tools expect configuration files in specific locations (usually the repository
root). Using symlinks:
...
The following file types are excluded from version control but may exist
locally:
...
These files provide project-specific context and coding standards to AI
assistants.
